...
func write_text(c: DialogCharacter, t: String) -> DialogLabel:
...
func choice(c: DialogCharacter, t: String, choices: Array[String]):
	var buttons := []
	...
	await write_text(c, t).finished
	
	for btn in buttons:
		await get_tree().create_timer(0.5).timeout
		$Text.add_child(btn)

func write(c: DialogCharacter, t: String):
	await write_text(c, t).finished
...
func _ready():
	...
	chose.connect(func(v: int):
		if v == 0:
			...
			write(TEST_CHAR, "no fun"), CONNECT_ONE_SHOT)
